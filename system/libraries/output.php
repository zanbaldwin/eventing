 ...
 */
	...
	class output extends library {
		 ...
		 * Status Code
		 *
		 * Set the HTTP/1.1 status code, and an optional message, if a message is not set, it will be determined from
		 * the status code.
		 *
		 * @access public
		 * @param integer $code
		...
		public function status($code, $message = false) {
			...
			$code = (int) $code;
			// We have a pretty exhaustive list of status codes. You shouldn't be passing any others, if you really
			// REALLY need to, add them to the array above.
		...
		public function display($continue = false) {
			...
			// Do we want to continue running the application, or should we terminate now the client has got what they requested?
			if($continue) {
				// Reset the buffer. No point keeping the output if it has already been sent to the client.
				$this->output = '';
				return true;
			}
